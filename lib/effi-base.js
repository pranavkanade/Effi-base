...
export default {
	...
	provideLinter() {
		...
		new_err = this.new_operator(read);

		var js_result = {
			"lines_Of_Code": loc,
			"infinite_loop": infinite_count,
			"Namespace_count": namespace_count,
			"Operation_with_1": operators,
			"Multiple_different_headers": multi_header,
			"Multiple_return_present": multi_returns,
			"Number_of_functions_uncommented": number_of_uncomments,
			"Number_of_goto_used": number_of_goto,
			"memory_allocation_leaks": new_err,
			"Tips_for_the_programmer": tips
		};
	...
	new_operator: function(read) {
		var count, del_var, error, fs, i, index, index_equal, index_terminate, input, j, k, l, len1, len2, len3, len4, m, matches_delete, matches_new, n, new_var, read, variable, x;
		new_var = [];
		del_var = [];
		//fs = require('fs');
		//input = filename;
		...
		try {
			count = 0;
			matches_new = read.match(/int(\s)*\*(\s)*[a-zA-Z0-9_$*;]+(\s)*=(\s)*new(\s)*int(\s)*(\t)*(\n)*;/g);
			matches_delete = read.match(/delete(\s)*(\t)*(\n)*[a-zA-Z0-9_$*;]+(\s)*(\t)*(\n)*/g);
			for (k = 0, len1 = matches_new.length; k < len1; k++) {
				i = matches_new[k];
				index = i.indexOf("*");
				index_equal = i.indexOf("=");
				variable = i.substr(index + 1, index_equal - index - 1);
				new_var.push(variable);
			}
			for (l = 0, len2 = matches_delete.length; l < len2; l++) {
				j = matches_delete[l];
				index = 6;
				index_terminate = j.indexOf(";");
				variable = j.substr(index + 1, index_terminate - index - 1);
				del_var.push(variable);
			}
			if (new_var.length > del_var.length | new_var.length < del_var.length) {
				tips.push("Please give a check on your memory allocation and deallocation");
			} else {
				for (m = 0, len3 = new_var.length; m < len3; m++) {
					i = new_var[m];
					for (n = 0, len4 = del_var.length; n < len4; n++) {
						j = del_var[n];
						if (i === j) {
							count = count + 1;
						}
			...
			if (matches_new.length < matches_delete.length) {
				tips.push("It seems you want to delete some unrecognized memory");
				return matches_delete.length - count;
			} else if (count === matches_new.length) {
				return console.log("All memory allocated and deallocated properly");

			} else {
				tips.push("Freeing of all the allocated memory is recommended");
				x = matches_new.length - count;
				if (x >= 0) {
					return x;
				} else {
					return 0;
	...
	lineOfCode: function(read) {
		...
		try {
			...
			fun1 = read.match(/(int|void|double|float)(\s)*(\t)*[a-zA-Z0-9_]*(\s)*(\t)*(\n)*\((\s)*(\t)*[a-zA-Z\s\t\n,:.]*(\s)*(\t)*(\n)*\)(\s)*(\t)*(\n)*\{(\s)*(\t)*(\n)*[a-zA-Z0-9\+\-\*%\/=<>;:\s\t\n"]*\}/g);
			flag = 0;
	...
	commenting: function(read) {
		...
		try {
			...
			matches = read.match(/\/\*[a-zA-Z0-9_$\t\s\n\+\-\*\/<>;,.:]+(\s)*(\t)*(\n)*\*\//g);
			functions = read.match(/(int|void|double|float)(\s)*(\t)*(\n)*[a-zA-Z0-9_$:\s\t\n]*(\s)*(\t)*(\n)*\((\s)*(\t)*(\n)*[a-zA-Z_$\s\t\n,:.]*(\s)*(\t)*(\n)*\)(\s)*(\t)*(\n)*\{(\s)*(\t)*(\n)*[a-zA-Z0-9_$\+\-\*\/=<>;:\s\t\n\."\[\]\(\)]*\}/g);
