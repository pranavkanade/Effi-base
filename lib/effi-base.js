	...
	push_to_json(clang_message) {
		...
		fs.readFile("/home/farzicoder/EffiTesting/session_result.json", 'utf8', function callback(err, data) {
			if (err) {
				...
				// 	}
				// });
				var length_of_sessions_result = obj_result.session_result.length;
				var recent_date_of_analysis = obj_result.session_result[length_of_sessions_result - 1];
				if (recent_date_of_analysis.Date !== curr_date) {
					obj_result.push({
						"Date": curr_date,
						"sessions": []
					});
					recent_date_of_analysis = obj_result.session_result[length_of_sessions_result];
				}
				recent_date_of_analysis.sessions.push({
					"time": curr_time,
					"num_of_warnings": warnings,
